...
from classes import stats as controller
...
class Overlay_info(QWidget):
    def __init__(self, api_client, offset_x=0):
        ...
        print(type(self.api_client))
        print(dir(self.api_client))
        print(hasattr(self.api_client, "is_auth"))
...
class Stats(QWidget):   
    ...
    def set_value(self, key, value):
        ...
        if key in self.labels:
            ...
            if key == "Победы" and value != '-':
                ...
                wins = mainStats.getWins()
                losses = mainStats.getLosses()
    ...
    def update_stats_periodically(self):
        while True:
            try:
                success = self.api_client.update_main_stats()
                if success:
                    mainStats = self.api_client.getMainStats()
                    ratingStats = self.api_client.getRatingStats()
                    self.set_value("Золото", str(mainStats.getGold()))
                    self.set_value("Кредиты", str(mainStats.getCredits()))
                    self.set_value("Боевой опыт", str(mainStats.getExpBattle()))
                    self.set_value("Cвободный опыт", str(mainStats.getExpFree()))
                    self.set_value("Проведено боев", str(mainStats.getBattles()))
                    self.stream_page.set_value("Бои", str(mainStats.getBattles() + ratingStats.getBattles()))
                    if mainStats.getBattles() > 0:
                        self.set_value("Победы", str(mainStats.getPercentWins()))
                        self.set_value("Урон", str(mainStats.getAvgDamage()))
                        self.set_value("Опыт", str(mainStats.getAvgExp()))
                    else:
                        self.set_value("Победы", "-")
                        self.set_value("Урон", "-")
                        self.set_value("Опыт", "-")

                    if mainStats.getBattles() > 0 or ratingStats.getBattles() > 0:
                        self.stream_page.set_value("Победы", str(round(((mainStats.getWins() + ratingStats.getWins()) / (mainStats.getBattles() + ratingStats.getBattles())) * 100.00, 2)))
                        self.stream_page.set_value("Урон", str((mainStats.getTotalDamage() + ratingStats.getTotalDamage()) // (mainStats.getBattles() + ratingStats.getBattles())))
                        self.stream_page.set_value("Опыт", str((mainStats.getExpBattle() + ratingStats.getExpBattle()) // (mainStats.getBattles() + ratingStats.getBattles())))
                    else:
...
class Rating(QWidget): 
    ...
    def set_value(self, key, value):
        ...
        if key in self.labels:
            ...
            if key == "Победы" and value != '-':
                ...
                ratingStats = self.api_client.getRatingStats()
                wins = ratingStats.getWins()
                losses = ratingStats.getBattles() - ratingStats.getWins()
                value = f"{value} [{wins}-{losses}]"
            dots = '.' * max(1, max_length - len(key) - len(value))
    ...
    def update_stats_periodically(self):
        while True:
            try:
                success = self.api_client.update_rating_stats()
                if success:
                    ratingStats = self.api_client.getRatingStats()
                    self.set_value("Текущий рейтинг", str(ratingStats.getRating()))
                    self.set_value("Прогресс рейтинга", str(ratingStats.getDiffRating()))
                    self.set_value("Калибровочные бои", str(ratingStats.getCalibBattles()))
                    self.set_value("Проведено боев", str(ratingStats.getBattles()))
                    if ratingStats.getBattles() > 0:
                        self.set_value("Победы", str(ratingStats.getPercentWins()))
                        self.set_value("Урон", str(ratingStats.getAvgDamage()))
                        self.set_value("Опыт", str(ratingStats.getAvgExp()))
                    else:
                        self.set_value("Победы", "-")
                        self.set_value("Урон", "-")
                        self.set_value("Опыт", "-")

                    if ratingStats.getDiffRating() >= 0:
                        self.stream_page.set_value("Рейтинг", str(ratingStats.getRating()) + " (+" + str(ratingStats.getDiffRating()) + ")")
                    else:
                        self.stream_page.set_value("Рейтинг", str(ratingStats.getRating()) + " (" + str(ratingStats.getDiffRating()) + ")")
                else:
                    print("Не удалось обновить статистику.")
...
class Other(QWidget):   
    ...
    def update_masters_periodically(self):
        while True:
            try:
                success = self.api_client.update_mastery_stats()
                if success:
                    masteryStats = self.api_client.getMasteryStats()
                    masteryData = masteryStats.getCurrentData()
                    self.set_value_masters("master_label", str(masteryData.mastery))
                    self.set_value_masters("master_label_1", str(masteryData.mastery_1))
                    self.set_value_masters("master_label_2", str(masteryData.mastery_2))
                    self.set_value_masters("master_label_3", str(masteryData.mastery_3))
                else:
...
class Stream(QWidget):   
    ...
    def rebuild_grid(self):
        ...
        mainStats = self.api_client.getMainStats()
        ratingStats = self.api_client.getRatingStats()
        masteryData = self.api_client.getMasteryStats().getCurrentData()
        otherStats = self.api_client.getOtherStats()

        battles = mainStats.getBattles() + ratingStats.getBattles()
        if battles > 0:
            wins = mainStats.getWins() + ratingStats.getWins()
            total_damage = mainStats.getTotalDamage() + ratingStats.getTotalDamage()
            exp = mainStats.getExpBattle() + ratingStats.getExpBattle()
            master = masteryData.mastery
            shots = otherStats.getPercentHits()
            survived = otherStats.getPercentSurvived()
            ...
            rating_delta = ratingStats.getDiffRating()
            sign = "+" if rating_delta >= 0 else ""
            values["Рейтинг"] = f"{ratingStats.getRating()} ({sign}{rating_delta})"
        else:
            values = {
                "Бои": battles,
                "Победы": "-",
...
class Info(QWidget):
    ...
    def setup_ui(self):
        ...
        if(self.api_client.is_auth()):
            self.message_text.setText(f"{self.api_client.getNickname()}")
        else:
    ...
    def server_auth(self):
        if self.api_client.is_auth():
            ...
            def auth_thread():
                ...
                if success:
                    while not self.api_client.is_auth():
                        time.sleep(0.1)

                    nickname = self.api_client.getNickname()
...
class MainApp:
    def __init__(self):
        ...
        self.api_client = controller.ApiController()
        ...
        if not HWIDActivator.check_activation():
            ...
            current_date = datetime.now()
            expiration_date = datetime(2025, 8, 30, 23, 59, 59)
